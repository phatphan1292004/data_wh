...
async function reportMovieStats() {
  const totalMovies = await queryWarehouse('SELECT COUNT(*) as count FROM fact_movie');
  const genres = await queryWarehouse('SELECT genre_name, COUNT(*) as count FROM dim_genre JOIN bridge_movie_genre ON dim_genre.genre_key = bridge_movie_genre.genre_key GROUP BY genre_name ORDER BY count DESC LIMIT 10');
  const countries = await queryWarehouse('SELECT country_name, COUNT(*) as count FROM dim_country JOIN bridge_movie_country ON dim_country.country_key = bridge_movie_country.country_key GROUP BY country_name ORDER BY count DESC LIMIT 10');
